...
MM_StandardAccessBarrier::preMountContinuation(J9VMThread *vmThread, j9object_t contObject)
{
	...
	j9tty_printf(PORTLIB, "preMountContinuation contObject=%p, vmThread=%p, _extensions->isConcurrentScavengerInProgress()=%zu, isCurrentPhaseConcurren()t=%zu, readBarrierRangeCheckTop=%p, readBarrierRangeCheckBase=%p\n",
			contObject, vmThread, _extensions->isConcurrentScavengerInProgress(), _extensions->scavenger->isCurrentPhaseConcurrent(), vmThread->readBarrierRangeCheckTop, vmThread->readBarrierRangeCheckBase);
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
...
MM_StandardAccessBarrier::postUnmountContinuation(J9VMThread *vmThread, j9object_t contObject)
{
	 ...
	 */
	PORT_ACCESS_FROM_VMC(vmThread);
	j9tty_printf(PORTLIB, "postUnmountContinuation contObject=%p, isOld=%zu, vmThread=%p\n", contObject, _extensions->isOld(contObject), vmThread);
