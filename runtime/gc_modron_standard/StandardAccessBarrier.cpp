...
{
//	if (J9_ARE_ANY_BITS_SET(vmThread->privateFlags, J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE)) {
//		/* concurrent scavenger in progress */
//		PORT_ACCESS_FROM_VMC(vmThread);
//		J9VMContinuation *continuation = J9VMJDKINTERNALVMCONTINUATION_VMREF(vmThread, contObject);
//		j9tty_printf(PORTLIB, "MM_StandardAccessBarrier::postMountContinuation vmThread=%p, contObject=%p, continuation=%p\n", vmThread, contObject, continuation);
...
//		PORT_ACCESS_FROM_VMC(vmThread);
//		J9VMContinuation *continuation = J9VMJDKINTERNALVMCONTINUATION_VMREF(vmThread, contObject);
//		j9tty_printf(PORTLIB, "MM_StandardAccessBarrier::preUnmountContinuation vmThread=%p, contObject=%p, continuation=%p\n", vmThread, contObject, continuation);
//		MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(vmThread->omrVMThread);
//		MM_ScavengeScanReason reason = SCAN_REASON_SCAVENGE;
//		_scavenger->getDelegate()->scanContinuationNativeSlots(env, contObject, reason, true);
//	}
//#endif /* OMR_GC_CONCURRENT_SCAVENGER */
}
