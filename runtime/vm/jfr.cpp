...
writeOutGlobalBuffer(J9VMThread *currentThread, bool finalWrite, bool dumpCalled)
{
	...
	if (vm->jfrState.isCreated && (NULL != vm->jfrBuffer.bufferCurrent)) {
		...
		j9tty_printf(PORTLIB, "writeOutGlobalBuffer flushJFRDataToFile currentThread=%p vm->jfrBuffer.bufferRemaining=%zu, vm->jfrBuffer.bufferCurrent=%zu\n", currentThread, vm->jfrBuffer.bufferRemaining, vm->jfrBuffer.bufferCurrent);
	...
	*hasExclusive = (J9_XACCESS_EXCLUSIVE == vm->exclusiveAccessState) || (J9_XACCESS_EXCLUSIVE == vm->safePointState);
	...
	if (vm->jfrBuffer.bufferRemaining < size) {
		if (isJFRV2SupportEnabled(vm)) {
			...
			j9tty_printf(PORTLIB, "allocateMemFromGlobalBuffer writeOutGlobalBuffer currentThread=%p hasExclusive=%zu, vm->jfrBuffer.bufferRemaining=%zu, size=%zu\n", currentThread, *hasExclusive, vm->jfrBuffer.bufferRemaining, size);
	...
	j9tty_printf(PORTLIB, "allocateMemFromGlobalBuffer currentThread=%p, vm->jfrBuffer.bufferRemaining=%zu, vm->jfrBuffer.bufferCurrent=%zu, bytes=%zu\n",
			currentThread, vm->jfrBuffer.bufferRemaining, vm->jfrBuffer.bufferCurrent, bytes);
...
flushBufferToGlobal(J9VMThread *currentThread, J9VMThread *flushThread, bool flushJavaBuffer)
{
	...
	j9tty_printf(PORTLIB, "flushBufferToGlobal allocateMemFromGlobalBuffer currentThread=%p flushThread=%p, flushJavaBuffer=%zu\n", currentThread, flushThread, flushJavaBuffer);
...
reserveBuffer(J9VMThread *currentThread, J9VMThread *sampleThread, UDATA size)
{
	...
	bool areBuffersReady = areJFRBuffersReadyForWrite(sampleThread);
	bool isJFRDisabledOnThread = isJFRRecordingDisabledOnThread(currentThread, sampleThread);

	if (!areBuffersReady || isJFRDisabledOnThread) {
		goto done;
	}
	...
	if (size <= sampleThread->jfrBuffer.bufferSize) {
		...
		if (size > sampleThread->jfrBuffer.bufferRemaining) {
			...
			PORT_ACCESS_FROM_VMC(currentThread);
			j9tty_printf(PORTLIB, "reserveBuffer flushBufferToGlobal currentThread=%p sampleThread=%p, sampleThread->jfrBuffer.bufferRemaining=%zu, size=%zu\n", currentThread, sampleThread, sampleThread->jfrBuffer.bufferRemaining, size);
			...
			j9tty_printf(PORTLIB, "reserveBuffer flushBufferToGlobal currentThread=%p", currentThread);
			if (!flushRC) {
				goto done;
			}
		}
	...
	if (NULL == jfrEvent) {
		Trc_VM_jfrReserveBuffer_nullBuffer(currentThread, sampleThread, size, areBuffersReady, isJFRDisabledOnThread);
	}
	return jfrEvent;
}
...
jfrThreadCreated(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
	...
	if (NULL == currentThread->jfrBuffer.bufferStart) {
 ...
 *
 * Flushes all thread buffers. Current thread has exclusive VM access.
 *
 * @param hook[in] the VM hook interface
 * @param eventNum[in] the event number
 * @param eventData[in] the event data
...
#if JAVA_SPEC_VERSION >= 17
/* Returns whether the JFR event identified by jfrEventId is currently enabled. */
static BOOLEAN
isJFREventEnabled(J9JavaVM *vm, jlong jfrEventId)
{
	BOOLEAN result = TRUE;
	if (isJFRV2SupportEnabled(vm)) {
		result = (NULL != vm->jfrState.jfrEventEnabledFlags)
			&& (0 <= jfrEventId)
			&& (jfrEventId < vm->jfrState.jfrEventEnabledFlagsSize)
			&& vm->jfrState.jfrEventEnabledFlags[(UDATA)jfrEventId];
	}
	return result;
}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
jfrThreadStarting(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadStartEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	 ...
	 */
	if (isJFRRecordingDisabledOnThread(currentThread, startedThread)) {
		return;
	}
...
jfrThreadEnd(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
	...
	if (
#if JAVA_SPEC_VERSION >= 17
		isJFREventEnabled(currentThread->javaVM, JfrThreadEndEvent)
...
#if JAVA_SPEC_VERSION == 17
	/* ThreadSleep is not defined in the JFR metadata for other JDK versions. */
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadSleepEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION == 17 */
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrJavaMonitorWaitEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrJavaMonitorEnterEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadParkEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrSystemGCEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
jfrOldGarbageCollection(OMR_VMThread *omrVMThread)
{
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(javaVM, JfrOldGarbageCollectionEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	...
	if (NULL != jfrEvent) {
		...
		jfrEvent->duration = endTicks - jfrEvent->startTicks;
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(javaVM, JfrYoungGarbageCollectionEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(javaVM, JfrGarbageCollectionEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
jfrObjectAllocationSample(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
	...
	if (NULL != jfrEvent) {
		...
		PORT_ACCESS_FROM_VMC(currentThread);
		j9tty_printf(PORTLIB, "jfrObjectAllocationSample currentThread=%p, sampleCount=%zu\n", currentThread, sampleCount);
	}
}
...
//
//	j9tty_printf(PORTLIB, "startJFRRecording enable JFRObjectAllocationSample rc=%zu, currentThread=%p,  objectAllocationSampleThrottleRate=%zu, jfr_allocation_sampling_interval=%zu\n", rc, currentThread,
//			vm->jfrState.objectAllocationSampleThrottleRate, J9JFR_OBJECT_ALLOCATION_SAMPLE_DEFAULT_INTERVAL);
	return rc;
}
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrExecutionSampleEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrCPULoadEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	PORT_ACCESS_FROM_VMC(currentThread);
...
jfrThreadCPULoad(J9VMThread *currentThread, J9VMThread *sampleThread)
{
	...
	if (-1 != rc) {
		J9JFRThreadCPULoad *jfrEvent = (J9JFRThreadCPULoad *)reserveBuffer(currentThread, sampleThread, sizeof(J9JFRThreadCPULoad));
		if (NULL != jfrEvent) {
			initializeEventFields(currentThread, sampleThread, (J9JFREvent *)jfrEvent, J9JFR_EVENT_TYPE_THREAD_CPU_LOAD);
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadCPULoadEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrClassLoadingStatisticsEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	J9JavaVM *vm = currentThread->javaVM;
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadContextSwitchRateEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	PORT_ACCESS_FROM_VMC(currentThread);
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrNetworkUtilizationEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	PORT_ACCESS_FROM_VMC(currentThread);
...
{
#if JAVA_SPEC_VERSION >= 17
	if (!isJFREventEnabled(currentThread->javaVM, JfrJavaThreadStatisticsEvent)) {
		return;
	}
#endif /* JAVA_SPEC_VERSION >= 17 */
	J9JavaVM *vm = currentThread->javaVM;
...
isJFRRecordingDisabledOnThread(J9VMThread *currentThread, J9VMThread *sampleThread)
{
	...
	J9JavaVM *vm = currentThread->javaVM;
	if (isJFRV2SupportEnabled(vm)) {
		j9object_t threadObject = sampleThread->threadObject;
		if (NULL != threadObject) {
			rc =  J9OBJECT_I32_LOAD(currentThread, threadObject, vm->isJFRExcludedOffset);
		}
	}
	return rc;
}
...
initializeJFRIDs(J9JavaVM *vm)
{
	...
	vm->jfrState.jfrEventEnabledFlags = NULL;
	vm->jfrState.jfrEventEnabledFlagsSize = 0;
...
shutdownJFRIDs(J9JavaVM *vm)
{
	...
	if (NULL != vm->jfrState.jfrEventEnabledFlags) {
		PORT_ACCESS_FROM_JAVAVM(vm);
		j9mem_free_memory(vm->jfrState.jfrEventEnabledFlags);
		vm->jfrState.jfrEventEnabledFlags = NULL;
		vm->jfrState.jfrEventEnabledFlagsSize = 0;
	}
}
...
getTypeIdImpl(J9VMThread *currentThread, J9ClassLoader *classLoader, J9UTF8 *className, BOOLEAN freeName, J9Class *clazz)
{
	...
	if (NULL == typeIDTable) {
		...
		if (vm->systemClassLoader == classLoader) {
			...
			/* Allocate enabled-flag array indexed by JFR event/type ID. */
			jlong jfrEventEnabledFlagsArraySize = vm->jfrState.typeIDcount + 1;
			UDATA jfrEventEnabledFlagsArrayBytes = (UDATA)(jfrEventEnabledFlagsArraySize * sizeof(U_8));
			U_8 *jfrEventEnabledFlagsArray = (U_8 *)j9mem_allocate_memory(jfrEventEnabledFlagsArrayBytes, J9MEM_CATEGORY_JFR);
			if (NULL == jfrEventEnabledFlagsArray) {
				hashTableFree(typeIDTable);
				classLoader->typeIDs = NULL;
				setNativeOutOfMemoryError(currentThread, 0, 0);
				goto done;
			}
			memset(jfrEventEnabledFlagsArray, 0, jfrEventEnabledFlagsArrayBytes);
			vm->jfrState.jfrEventEnabledFlags = jfrEventEnabledFlagsArray;
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrPhysicalMemoryEvent)) {
		return;
	}
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrJVMInformationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrOSInformationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrVirtualizationInformationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrInitialSystemPropertyEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrInitialEnvironmentVariableEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrSystemProcessEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrCPUInformationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
JfrPeriodicEventSet::requestThreadDump(J9VMThread *currentThread)
{
	...
	if (!isJFREventEnabled(vm, JfrThreadDumpEvent)) {
		return;
	}
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrNativeLibraryEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrModuleRequireEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrModuleExportEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrGCHeapConfigurationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrYoungGenerationConfigurationEvent)) {
		return;
	}
	J9JFREvent *jfrEvent = (J9JFREvent *)reserveBuffer(currentThread, currentThread, sizeof(J9JFREvent));
	if (NULL != jfrEvent) {
...
{
	if (!isJFREventEnabled(currentThread->javaVM, JfrThreadAllocationStatisticsEvent)) {
		return;
	}
	J9JavaVM *vm = currentThread->javaVM;
	J9MemoryManagerFunctions *mmfns = vm->memoryManagerFunctions;
