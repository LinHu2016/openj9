...
walkMethodFrame(J9StackWalkState * walkState)
{
	...
	BOOLEAN isJNINative = ((UDATA)walkState->pc == J9SF_FRAME_TYPE_JNI_NATIVE_METHOD);
	...
	if ((walkState->flags & J9_STACKWALK_ITERATE_O_SLOTS) && walkState->literals) {
		if ((walkState->frameFlags & J9_SSF_JNI_REFS_REDIRECTED) || (isJNINative && walkState->literals)) {
