...
//			((MM_AllocationContextBalanced *)envBase->_objectAllocationInterface)->needRecycleLeafRegionFraction(envBase, fraction);
		...
		j9tty_printf(PORTLIB, "getSparseAddressAndDecommitLeaves fraction=%zu, arrayReservedRegionCount=%zu, getArrayReservedRegionCount=%zu, getFreeRegionCount=%zu\n",
				fraction, arrayReservedRegionCount, commonContext->getArrayReservedRegionCount(), context->getFreeRegionCount());
	}
