...
MM_GCExtensions::releaseNativesForContinuationObject(MM_EnvironmentBase* env, j9object_t objectPtr)
{
	...
	if (verify_continuation_list == continuationListOption) {
		...
		/* there might be potential case that GC would happen between JVM_VirtualThreadUnmountBegin() and JVM_VirtualThreadUnmountEnd() and last unmounted continuation Object is marked as "dead", but the related J9VMContinuation has not been freed up. */
		if (!finished) {
