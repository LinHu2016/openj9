...
MM_ContinuationObjectBuffer::MM_ContinuationObjectBuffer(MM_GCExtensions *extensions, UDATA maxObjectCount)
	: MM_BaseVirtual()
...
void
MM_ContinuationObjectBuffer::kill(MM_EnvironmentBase *env)
{
	tearDown(env);
...
void
MM_ContinuationObjectBuffer::reset()
{
	_head = NULL;
	_tail = NULL;
	_region = NULL;
...
MM_ContinuationObjectBuffer::add(MM_EnvironmentBase* env, j9object_t object)
{
	...
	if ( (_objectCount < _maxObjectCount) && _region->isAddressInRegion(object) ) {
		...
		if (NULL == region || !region->isAddressInRegion(object)) {
			...
			Assert_GC_true_with_message(env, NULL != region, "Attempt to accesscontinuation object located outside of heap (stack allocated?) %p\n", object);
		}
		_region = region;
