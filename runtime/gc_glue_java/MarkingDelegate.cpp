...
MM_MarkingDelegate::scanContinuationObject(MM_EnvironmentBase *env, omrobjectptr_t objectPtr)
{
	...
	if (started && (NULL != j9vmContinuation)) {
		...
		GC_VMThreadStackSlotIterator::scanSlots(currentThread, &continuationThread, (void *)&localData, stackSlotIterator4SMarkingDelegate, bStackFrameClassWalkNeeded, false);
		/*debug*/
