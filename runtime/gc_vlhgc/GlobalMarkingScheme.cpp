		...
		if (!_markingScheme->isMarked(objectPtr)) {
			...
			if (NULL != dataAddr) {
				...
				if (0 != fraction) {
					...
					PORT_ACCESS_FROM_ENVIRONMENT(env);
					j9tty_printf(PORTLIB, "MM_GlobalMarkingSchemeRootClearer::doObjectInVirtualLargeObjectHeap recycleToSharedArrayReservedRegion env=%p, objectPtr=%p, context=%p, fraction=%zu, reservedRegionCount=%zu\n",
							env, objectPtr, context, fraction, reservedRegionCount);
