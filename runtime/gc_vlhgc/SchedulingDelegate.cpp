...
MM_SchedulingDelegate::calculateEdenSize(MM_EnvironmentVLHGC *env, bool allowTotalHeapResize)
{
	...
	PORT_ACCESS_FROM_ENVIRONMENT(env);
	j9tty_printf(PORTLIB, "calculateEdenSize allowTotalHeapResize=%d, desiredEdenChangeSize=%d, maxHeapExpansionRegions=%d, maxEdenChange=%d, _edenRegionCount=%zu, freeRegions=%zu\n",
			allowTotalHeapResize, desiredEdenChangeSize, maxHeapExpansionRegions, maxEdenChange, _edenRegionCount, freeRegions);
	/**
