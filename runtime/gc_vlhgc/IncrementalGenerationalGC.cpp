...
//		_extensions->globalVLHGCStats._heapSizingData.freeTenure -= edenSize - allocatedSinceLastPGC;
		...
		j9tty_printf(PORTLIB, "freeTenure=%zu, freeMemoryForSurvivor=%zu, edenSize=%zu, allocatedSinceLastPGC=%zu, edenSize - allocatedSinceLastPGC=%zu\n",
				freeMemoryForSurvivor - (edenSize - allocatedSinceLastPGC),
				freeMemoryForSurvivor,
				edenSize,
				allocatedSinceLastPGC,
				(edenSize - allocatedSinceLastPGC));
...
{
	Assert_GC_true_with_message4(env, (((MM_GlobalAllocationManagerTarok *)_extensions->globalAllocationManager)->getFreeRegionCount() >= _schedulingDelegate.getCurrentEdenSizeInRegions(env)),
