...
MM_IncrementalGenerationalGC::preMainGCThreadInitialize(MM_EnvironmentBase *envBase)
{
	...
	if (0 == envBase->_freeEntrySizeClassStats.getMaxSizeClasses()) {
		MM_LargeObjectAllocateStats::initializeFreememoryProfileMaxSizeClasses(envBase, _extensions->largeObjectAllocationProfilingVeryLargeObjectThreshold,
				(float)_extensions->largeObjectAllocationProfilingSizeClassRatio / (float)100.0, _extensions->heap->getMaximumMemorySize());
		envBase->_freeEntrySizeClassStats.tearDown(envBase);
		uintptr_t veryLargeObjectThreshold = (_extensions->largeObjectAllocationProfilingVeryLargeObjectThreshold <= _extensions->memoryMax)?0:_extensions->largeObjectAllocationProfilingVeryLargeObjectThreshold;
		envBase->_freeEntrySizeClassStats.initialize(envBase, _extensions->largeObjectAllocationProfilingTopK, _extensions->freeMemoryProfileMaxSizeClasses, veryLargeObjectThreshold);
	}
