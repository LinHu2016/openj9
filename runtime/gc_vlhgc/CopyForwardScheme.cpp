...
void
MM_CopyForwardScheme::updateCollectionSet(MM_EnvironmentVLHGC *env)
{
	if (0 == _regionCountReservedNonEvacuated) {
		return;
	}
	GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
	MM_HeapRegionDescriptorVLHGC *region = NULL;
	UDATA avgSurvivorSizePerEdenRegion = 0;
	UDATA additionalFreeSzie = 0;
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		if(region->containsObjects()) {
			if (region->_markData._shouldMark) {
				if (0 < _regionCountReservedNonEvacuated) {
					 if ((!region->isEden()) && MM_CompactGroupManager::isRegionInNursery(env, region)) {
...
//						 region->_defragmentationTarget = true;
						 ...
						 if (0 == region->getLogicalAge()) {
							 if (0 == avgSurvivorSizePerEdenRegion) {
//								 avgSurvivorSizePerEdenRegion = (UDATA)(region->getSize() * static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_schedulingDelegate->getAvgEdenSurvivalRateCopyForward(env));
								 avgSurvivorSizePerEdenRegion = (UDATA)(region->getSize() * persistentStats[0]._historicalSurvivalRate);
							 }
							additionalFreeSzie += ((MM_MemoryPoolAddressOrderedList*)region->getMemoryPool())->getActualFreeMemorySize();
							UDATA regionCount = additionalFreeSzie / avgSurvivorSizePerEdenRegion;
							if (regionCount >= _regionCountReservedNonEvacuated) {
								_regionCountReservedNonEvacuated = 0;
								break;
							} else {
								_regionCountReservedNonEvacuated -= regionCount;
								additionalFreeSzie %= avgSurvivorSizePerEdenRegion;
							}
						}
					 }
				} else {
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		...
		if(region->containsObjects()) {
			...
			if (region->_markData._shouldMark) {
				...
				if ((!env->_cycleState->_shouldRunCopyForward) || (region->_criticalRegionsInUse > 0) || (100 == _extensions->fvtest_forceCopyForwardHybridRatio) || (randomDecideForceNonEvacuatedRegion(_extensions->fvtest_forceCopyForwardHybridRatio))) {
					...
					if (_regionCountReservedNonEvacuated > 0) {
						_regionCountReservedNonEvacuated -= 1;
					}
...
//			PORT_ACCESS_FROM_ENVIRONMENT(env);
//			j9tty_printf(PORTLIB, "postProcessRegions AdjustedbytesForCardAlignment=%zu, region=%p, isFreshSurvivorRegion=%zu, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					pool->getAdjustedbytesForCardAlignment(), region, region->isFreshSurvivorRegion(), region->getLogicalAge(), pool->getActualFreeMemorySize(), pool->getActualFreeEntryCount(), pool->getDarkMatterBytes(), pool->getLargestFreeEntry());
//
...
//
//				j9tty_printf(PORTLIB, "clearReservedRegionLists region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());
//
//				memoryPool->reportMemoryPool(env);
...
//	PORT_ACCESS_FROM_ENVIRONMENT(env);
//	j9tty_printf(PORTLIB, "reserveMemoryForObject start compactGroup=%zu, sublistIndex=%zu, objectSize=%zu, regionList=%p\n", compactGroup, sublistIndex, objectSize, regionList);
//
...
//			memoryPool->verifyFreeSizeCount(env);
	//		PORT_ACCESS_FROM_ENVIRONMENT(env);
//			j9tty_printf(PORTLIB, "before alignMemoryPool4Collector AdjustedbytesForCardAlignment=%zu, region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					memoryPool->getAdjustedbytesForCardAlignment(), region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());
...
//			j9tty_printf(PORTLIB, "alignMemoryPool4Collector lostToAlignment=%zu, region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					lostToAlignment, region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());

			env->_copyForwardCompactGroups[compactGroup]._discardedBytes += lostToAlignment;
			if (0 == memoryPool->getActualFreeEntryCount()) {
//			if ((0 == memoryPool->getActualFreeEntryCount()) || (0 == memoryPool->getActualFreeMemorySize())) {
//				if (0 != memoryPool->getActualFreeMemorySize()) {
//					j9tty_printf(PORTLIB, "ActualFreeEntryCount=0, region=%p, ActualFreeMemorySize=%zu\n", region, memoryPool->getActualFreeMemorySize());
//				}
				Assert_MM_true(0 == memoryPool->getActualFreeMemorySize());

//				j9tty_printf(PORTLIB, "removeFreeMemoryCandidate region=%zu", region);
...
//
	...
	if (NULL != result) {
		...
		setCompressedSurvivorCards(env, (void*)result, (void*) ((UDATA)result + allocDescription.getBytesRequested()));
//		setCompressedSurvivorCards(env, (void*)result, (void*) ((UDATA)result + (1 << J9VMGC_SIZECLASSES_LOG_SMALLEST)));
	}
...
//
//	j9tty_printf(PORTLIB, "reserveMemoryForObject end result=%p, region=%p\n", result, region);
	return result;
}
...
//	PORT_ACCESS_FROM_ENVIRONMENT(env);
//	j9tty_printf(PORTLIB, "reserveMemoryForCache start compactGroup=%zu, sublistIndex=%zu, maxCacheSize=%zu, regionList=%p\n", compactGroup, sublistIndex, maxCacheSize, regionList);
//
	/* Measure the number of acquires before and after we acquire the lock. If it changed, then there is probably contention on the lock. */
	UDATA acquireCountBefore = regionList->_cacheAcquireCount;
	regionList->_lock.acquire();
...
//			memoryPool->reportMemoryPool(env);
//			PORT_ACCESS_FROM_ENVIRONMENT(env);
//			j9tty_printf(PORTLIB, "releaseRegion region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//				region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());
			releaseRegion(env, regionList, region);
		}
		region = next;
...
//			memoryPool->verifyFreeSizeCount(env);
//			PORT_ACCESS_FROM_ENVIRONMENT(env);
//			j9tty_printf(PORTLIB, "before alignMemoryPool4Collector TLH AdjustedbytesForCardAlignment=%zu, region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					memoryPool->getAdjustedbytesForCardAlignment(), region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());
//
			/* make sure that we can't be copying objects into the area covered by a card which is meant to describe objects which were already in the region */
			UDATA lostToAlignment = alignMemoryPool4Collector(env, memoryPool);

//			j9tty_printf(PORTLIB, "alignMemoryPool4Collector TLH lostToAlignment=%zu, region=%p, logicalAge=%zu, ActualFreeMemorySize=%zu, ActualFreeEntryCount=%zu, DarkMatterBytes=%zu, LargestFreeEntry=%zu\n",
//					lostToAlignment, region, region->getLogicalAge(), memoryPool->getActualFreeMemorySize(), memoryPool->getActualFreeEntryCount(), memoryPool->getDarkMatterBytes(), memoryPool->getLargestFreeEntry());
...
//				}
				Assert_MM_true(0 == memoryPool->getActualFreeMemorySize());
//				j9tty_printf(PORTLIB, "TLH removeFreeMemoryCandidate region=%zu", region);
...
//	if (result) {
//		j9tty_printf(PORTLIB, "reserveMemoryForCache end result=%zu, size=%zu, region=%p\n", result, (UDATA)(*addrTop) - (UDATA)(*addrBase), region);
//	} else {
//		j9tty_printf(PORTLIB, "reserveMemoryForCache end result=%zu\n", result);
//	}
	return result;
}
...
//	j9tty_printf(PORTLIB, "copyForwardCollectionSet _regionCountCannotBeEvacuated=%zu, shouldRunCopyForward=%zu\n", _regionCountCannotBeEvacuated, env->_cycleState->_shouldRunCopyForward);
//
...
//				PORT_ACCESS_FROM_ENVIRONMENT(env);
//				j9tty_printf(PORTLIB, "discardRemainingCache rebuildFreeListInRegion region=%p\n", env, region);
//			}
		}
...
//				if (!isLiveObject((J9Object *)classPtr->classObject)) {
//					PORT_ACCESS_FROM_ENVIRONMENT(env);
//					MM_HeapRegionDescriptorVLHGC *region = (MM_HeapRegionDescriptorVLHGC *)_regionManager->tableDescriptorForAddress(classPtr->classObject);
//					j9tty_printf(PORTLIB, "classPtr->classObject=%p, region=%p, isFreshSurvivorRegion=%zu, isSurvivorRegion=%zu\n", classPtr->classObject, region, region->isFreshSurvivorRegion(), region->isSurvivorRegion());
//					void *heapAddr = (void *)classPtr->classObject;
...
//					j9tty_printf(PORTLIB, "_heapBase=%p, compressedCardOffset=%zu, compressedCardIndex=%zu, compressedSurvivorWord=%zu, bit=%zu\n", _heapBase, compressedCardOffset, compressedCardIndex, compressedSurvivorWord, bit);
//				}
...
//		else {
//			PORT_ACCESS_FROM_ENVIRONMENT(env);
//			j9tty_printf(PORTLIB, "scanClassLoaderObjectSlots J9CLASSLOADER_ANON_CLASS_LOADER classLoaderObject=%p reason=%zu\n", classLoaderObject, reason);
//		}
	}
}	
...
//				j9tty_printf(PORTLIB, "MM_CopyForwardSchemeRootScanner::doClassLoader  classLoader=%p, classLoaderObject=%p, isBitSet=%c\n",
//						classLoader, classLoader->classLoaderObject, _env->_cycleState->_markMap->isBitSet(classLoader->classLoaderObject) ? 'Y' : 'N');
//				Assert_MM_true(_env->_cycleState->_markMap->isBitSet(classLoader->classLoaderObject));
//			}
//
		}
//		else {
//			if(J9_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
//				PORT_ACCESS_FROM_ENVIRONMENT(_env);
//				j9tty_printf(PORTLIB, "MM_CopyForwardSchemeRootScanner::doClassLoader J9_GC_CLASS_LOADER_DEAD classLoader=%p, classLoaderObject=%p, isBitSet=%c\n",
//						classLoader, classLoader->classLoaderObject, _env->_cycleState->_markMap->isBitSet(classLoader->classLoaderObject) ? 'Y' : 'N');
//
//			}
//		}
...
MM_CopyForwardScheme::alignMemoryPool4Collector(MM_EnvironmentVLHGC *env, MM_MemoryPoolAddressOrderedList *regionPool)
{
	...
	UDATA freeBytes = regionPool->getABSActualFreeMemorySize();
...
MM_CopyForwardScheme::scanRoots(MM_EnvironmentVLHGC* env)
{
	...
	if(isDynamicClassUnloadingEnabled()) {
		...
		if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
			...
			PORT_ACCESS_FROM_ENVIRONMENT(env);
			j9tty_printf(PORTLIB, "scanRoots env=%p, classLoaderBlocks=%p\n", env, _javaVM->classLoaderBlocks);
			while (NULL != (classLoader = classLoaderIterator.nextSlot())) {
				if (0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
					if(J9_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
						foundAnonymousClassLoader = true;

						j9tty_printf(PORTLIB, "scanRoots foundAnonymousClassLoader classLoader=%p, classLoaderObject=%p, isBitSet=%c\n",
								classLoader, classLoader->classLoaderObject, _markMap->isBitSet(classLoader->classLoaderObject) ? 'Y' : 'N');

//						if (!_markMap->isBitSet(classLoader->classLoaderObject)) {
...
//	UDATA usedBytes = region->getSize() - memoryPool->getActualFreeMemorySize() - memoryPool->getDarkMatterBytes();
	UDATA usedBytes = region->getSize() - memoryPool->getFreeMemoryAndDarkMatterBytes();
...
//	PORT_ACCESS_FROM_ENVIRONMENT(env);
//	j9tty_printf(PORTLIB, "setCompressedSurvivorCards startHeapAddress=%p, endHeapAddress=%p, compressedCardStartOffset=%zu, compressedCardEndOffset=%zu, compressedCardStartIndex=%zu, compressedCardEndIndex=%zu, endOfWord=%p, mask=%p\n",
//			startHeapAddress, endHeapAddress, compressedCardStartOffset, compressedCardEndOffset, compressedCardStartIndex, compressedCardEndIndex, endOfWord, mask);
...
//			j9tty_printf(PORTLIB, "idx=%zu, oldValue=%p, newValue=%p, endOfWord=%p\n", idx, oldValue, newValue, endOfWord);
			compressedSurvivor++;
