...
//			Assert_MM_mustBeClass(_extensions->objectModel.getPreservedClass(&forwardedHeader));
			...
			if (NULL == fwdOjectPtr) {
				...
				j9tty_printf(PORTLIB, "doObjectInVirtualLargeObjectHeap env=%p, objectPtr=%p, &forwardedHeader=%p, objectPtrHeader=%p, fwdOjectPtr=%p, dataAddr=%p\n",
							env, objectPtr, &forwardedHeader, *objectPtr, fwdOjectPtr, dataAddr);
				...
				*sparseHeapAllocation = false;
				...
				j9tty_printf(PORTLIB, "2doObjectInVirtualLargeObjectHeap env=%p,objectPtr=%p, &forwardedHeader=%p, objectPtrHeader=%p, fwdOjectPtr=%p, dataAddr=%p\n",
						env, objectPtr, &forwardedHeader, *objectPtr, fwdOjectPtr, dataAddr);
				if (NULL != dataAddr) {
