	...
	while (NULL != (region = regionIterator.nextRegion())) {
		if (region->containsObjects() && region->getRememberedSetCardList()->isAccurate()) {
			...
			if (!region->isEden()) {
				...
				if ((1 <= (100 * freeMemory)/regionSize) && ((NULL == _reservedFromCollectionSet[compactGroup]) ||
...
//				UDATA mostFreeMemory = ((MM_MemoryPoolBumpPointer *)region->getMemoryPool())->getActualFreeMemorySize();
				UDATA mostFreeMemory = ((MM_MemoryPoolBumpPointer *)region->getMemoryPool())->getLargestFreeEntry();
				if ((1 <= (100 * mostFreeMemory)/regionSize) && ((NULL == _reservedFromCollectionSetMostFree[compactGroup]) ||
	...
	for (UDATA idx =0; idx<compactGroupMaxCount; idx++) {
		...
		if (NULL != region) {
			...
			if ((reservedSize >= _projectedLiveBytes[idx]) || ((reservedSize < _projectedLiveBytes[idx]) && (MM_Math::roundToCeiling(regionSize, _projectedLiveBytes[idx]) <= MM_Math::roundToCeiling(regionSize, _projectedLiveBytes[idx] - reservedSize)))) {
		...
		if (NULL != region) {
			reservedSize = ((MM_MemoryPoolBumpPointer *)region->getMemoryPool())->getAllocatableBytes() + ((MM_MemoryPoolBumpPointer *)region->getMemoryPool())->getAllocatableBytes4Collector() + region->_projectedLiveBytes;
			if ((reservedSize >= _projectedLiveBytes[idx]) || ((reservedSize < _projectedLiveBytes[idx]) && (MM_Math::roundToCeiling(regionSize, _projectedLiveBytes[idx]) <= MM_Math::roundToCeiling(regionSize, _projectedLiveBytes[idx] - reservedSize)))) {
	...
	for (UDATA idx =0; idx<compactGroupMaxCount; idx++) {
		...
		if (NULL != region) {
			memoryPool = (MM_MemoryPoolBumpPointer *)region->getMemoryPool();
			j9tty_printf(PORTLIB, " compactGroup=%zu, id=%zu, free=%zu, ", idx, _regionManager->mapDescriptorToRegionTableIndex(region), memoryPool->getAllocatableBytes() + memoryPool->getAllocatableBytes4Collector());
		}
	}
	j9tty_printf(PORTLIB, "\n projectedLiveBytes :");
	for (UDATA idx =0; idx<compactGroupMaxCount; idx++) {
