...
public:
	...
	MMINLINE void abandonTLHRemainder(MM_EnvironmentVLHGC* env)
	{
		if (NULL != _TLHRemainderBase) {
			...
			MM_HeapRegionDescriptorVLHGC *region = (MM_HeapRegionDescriptorVLHGC*)extensions->heapRegionManager->tableDescriptorForAddress(_TLHRemainderBase);
			region->incrementTLHRemainderBytes(getTLHRemainderSize());
//			{
//				PORT_ACCESS_FROM_ENVIRONMENT(env);
//				j9tty_printf(PORTLIB, "abandonTLHRemainder preserve envID=%zu, RemainderBase=%p, RemainderSize=%zu, _gcIDForTLHRemainder=%zu, region=%p, isSurvivorRegion=%zu, DarkMatterBytes=%zu\n",
//					env->getEnvironmentId(), _TLHRemainderBase, getTLHRemainderSize(), _gcIDForTLHRemainder, region, region->isSurvivorRegion(), region->getMemoryPool()->getDarkMatterBytes());
//			}
...
//			incrementDarkMatterBytesForRemainder(region);
	...
	MMINLINE void recycleTLHRemainder(MM_EnvironmentVLHGC* env)
	{
		if (NULL != _TLHRemainderBase) {
			...
			pool->recycleHeapChunk(_TLHRemainderBase, _TLHRemainderTop);
			resetTLHRemainder();
		} else {
			Assert_MM_true(NULL == _TLHRemainderTop);
		}
...
//			if (_TLHRemainderBase < extensions->heap->getHeapBase()) {
//				PORT_ACCESS_FROM_ENVIRONMENT(env);
//				j9tty_printf(PORTLIB, "prepareTLHRemainder _TLHRemainderBase=%p, _TLHRemainderTop=%p, _gcIDForTLHRemainder=%zu, HeapBase=%p, compactGroup=%zu, envID=%zu\n",
//						_TLHRemainderBase, _gcIDForTLHRemainder, _TLHRemainderTop, extensions->heap->getHeapBase(), compactGroup, env->getEnvironmentId());
//			}
			MM_HeapRegionDescriptorVLHGC *region = (MM_HeapRegionDescriptorVLHGC*)extensions->heapRegionManager->tableDescriptorForAddress(_TLHRemainderBase);
...
//							PORT_ACCESS_FROM_ENVIRONMENT(env);
//							j9tty_printf(PORTLIB, "prepareTLHRemainder move envID=%zu, _RemainderBase=%p, RemainderSize=%zu, _gcIDForRemainder=%zu, region compactGroup=%zu, regionCompactGroup=%zu, destTLHRemainderSize=%zu, region=%p, lastGcIDForReclaim=%zu\n",
//									env->getEnvironmentId(), _TLHRemainderBase, getTLHRemainderSize(), _gcIDForTLHRemainder, compactGroup, regionCompactGroup, dest->getTLHRemainderSize(), region, region->_copyForwardData._lastGcIDForReclaim);
							if (0 != dest->getTLHRemainderSize()) {
//								dest->discardHeapChunk();
								dest->discardHeapChunk(dest->_TLHRemainderBase, dest->_TLHRemainderTop,  (MM_HeapRegionDescriptorVLHGC*)extensions->heapRegionManager->tableDescriptorForAddress(dest->_TLHRemainderBase));
							}
			...
			if (needResetTLHRemainder) {
//				discardHeapChunk();
				if ((NULL != region) && (_gcIDForTLHRemainder > region->_copyForwardData._lastGcIDForReclaim)) {
					discardHeapChunk(_TLHRemainderBase, _TLHRemainderTop, region);
				}
				resetTLHRemainder();
			}
