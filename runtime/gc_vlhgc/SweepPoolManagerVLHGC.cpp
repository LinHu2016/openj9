...
//	MM_SweepPoolState *result = ((MM_MemoryPoolBumpPointer *)memoryPool)->getSweepPoolState();
	MM_SweepPoolState *result = ((MM_MemoryPoolAddressOrderedList *)memoryPool)->getSweepPoolState();
...
//		MM_MemoryPoolBumpPointer *memoryPool = (MM_MemoryPoolBumpPointer *)memoryPoolBase;
		MM_MemoryPoolAddressOrderedList *memoryPool = (MM_MemoryPoolAddressOrderedList *)memoryPoolBase;
		if (!memoryPool->canMemoryBeConnectedToPool(envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate, sweepState->_connectPreviousChunk->trailingFreeCandidateSize)) {
			...
			sweepState->_sweepFreeBytes += sweepState->_connectPreviousFreeEntrySize;
			sweepState->_sweepFreeHoles += 1;
			sweepState->_largestFreeEntry = OMR_MAX(sweepState->_connectPreviousFreeEntrySize, sweepState->_largestFreeEntry);
		}
	}
}
...
//		if (((MM_MemoryPoolBumpPointer *)sweepChunk->memoryPool)->connectInnerMemoryToPool(env, address, freeSizeInBytes, sweepChunk->freeListTail)) {
		if (((MM_MemoryPoolAddressOrderedList *)sweepChunk->memoryPool)->connectInnerMemoryToPool(env, address, freeSizeInBytes, sweepChunk->freeListTail)) {
