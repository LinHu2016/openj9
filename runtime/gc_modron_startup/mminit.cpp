...
gcInitializeDefaults(J9JavaVM* vm)
{
	...
	while (true) {
		...
		if (JNI_OK == j9gc_initialize_heap(vm, memoryParameterTable, extensions->memoryMax)) {
			...
			PORT_ACCESS_FROM_JAVAVM(vm);
			j9tty_printf(PORTLIB, "j9gc_initialize_heap vm->isVirtualLargeObjectHeapEnabled=%zu, extensions->isVirtualLargeObjectHeapEnabled=%zu\n", vm->isVirtualLargeObjectHeapEnabled, extensions->isVirtualLargeObjectHeapEnabled);
