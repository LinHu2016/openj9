...
/* End of options relating to dynamicBreadthFirstScanOrdering */
		...
		if (try_scan(&scan_start, "disableScanMountedContinuationObject")) {
			extensions->disableScanMountedContinuationObject = true;
			continue;
		}

		if (try_scan(&scan_start, "enableContinuationMountingMutex")) {
			extensions->enableContinuationMountingMutex = true;
			continue;
		}
